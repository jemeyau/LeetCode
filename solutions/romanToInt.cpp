...
class Solution
{
  public:
    int romanToInt(string s)
    {
        ...
        {
            if (i+1 < s.size() && mapValue.find(s.substr(i, 2)) != mapValue.end())
            {
                //cout << "substr: " << s.substr(i, 2) << endl;
                ret += mapValue.find(s.substr(i, 2))->second;
                i++;
            }
            else
            {
                //cout << "substr2: " << s.substr(i, 1) << endl;
                ret += mapValue.find(s.substr(i, 1))->second;
            }
        }
